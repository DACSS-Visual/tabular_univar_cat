...
For example, when you visit the [website](https://nces.ed.gov/ccd/schoolsearch/) of the National Center for Education Statistics from the US Department of Education, you can get a data set with detailed information on public schools from the whole US. Let me get a data table I have prepared with schools from the State of Washington:
...
Whatever your visual goals are, you first need to know **how** that variable you are planning to use has been recognized in R:
...
> In real analysis, you'd pick a reference based on:

> - Policy benchmarks (e.g., "state average = 22%")
> - Research thresholds (e.g., "urban definition: >25% density")
> - Comparative baselines (e.g., national average)
>
> *Here, we use 25% simply to demonstrate the **technique** of adding reference lines. 
> In your own work, always justify your benchmarks!*
...
And, what about changing the axis values so that we can identify the gaps (using 25 as the reference as before):
...
Maybe add some color, for that we need to create a new categorical column to **tableFreq** data frame:
...
You should make  labels visible, and also  avoid the overlaping symbols in the legend (get rid of that redundancy):
...
barGrades=base + geom_col(aes(fill=pct>0.3)) 
# customize colors to use (are they a ggod choice?):
barGrades + scale_fill_manual(values=c("yellow","green"))
...
barGrades2=base + geom_col() 

# change the horizontal axis defaults
barGrades2 + theme(axis.text.x = element_text(color=TextAxisColor,size=9))
```
...
## 🔑 Key Takeaways
- **Nominal data**: Use bar/lollipop plots; order bars by value for clarity
- **Ordinal data**: Preserve natural order; consider boxplots to show asymmetry
- **Always**: Label missing values explicitly; test colors for accessibility
- **Reference lines**: Only add when you have a meaningful benchmark
...
# BONUS MATERIAL: Color blindness tests

We have prepared several plots, and the colors shown have been the default ones, or I have manually selected some colors. While you become savvy on choosing right colors, you could start exploring the library **colorblindr** (read documentation to install the package [here](https://github.com/clauswilke/colorblindr)).

This package offers to replot your viz showing it as if you have colorblindness.

See some of our plots:
...
This case uses default colors:
```{r, message=FALSE}
library(colorblindr)
cvd_grid(lolliplot8)
```

> 🔍 **Reading `cvd_grid()` output**:
> 
> Our goal: the visualization should remain interpretable in **all four panels**!
> 
> - **Deutanomaly**: Most common red-green colorblindness (~5% of males)
> - **Protanomaly**: Red-green deficiency (~1% of males)  
> - **Tritanomaly**: Blue-yellow deficiency (rare)
> - **Desaturated**: Simulates grayscale vision—critical for B&W printing, photocopying, and viewers with reduced color sensitivity


This case I selected the colors manually:
```{r}

cvd_grid(barGrades + scale_fill_manual(values=c("yellow","green")))
```

Try an alternative?
```{r}
cvd_grid(barGrades + scale_fill_manual(values=c("grey70", "#0072B2")))
```
Check the use of safe palettes:

- Brewer
```{r}
barGrades_brewer1=barGrades + 
...
```{r}
cvd_grid(barGrades_brewer1)
```
```{r}
barGrades_brewer2 <- barGrades + 
  scale_fill_brewer(palette = "Paired",
                    name = "High enrollment\n(>30%)")
barGrades_brewer2
```
```{r}
cvd_grid(barGrades_brewer2)

```

### ColorBrewer Palette Quick Reference

| Purpose | Recommended Palette | Colorblind-Safe? |
|---------|--------------------|-----------------|
| **Categorical (2-8 groups)** | `Set2`, `Dark2`, `Paired` | ✅ Yes |
| **Categorical (many groups)** | `Set3`, `Accent` | ✅ Yes |
| **Sequential (low→high)** | `Blues`, `Greys`, `Purples` | ✅ Yes |
| **Diverging (low↔high)** | `RdBu`, `PiYG` | ✅ Yes |
| **Diverging (avoid!)** | `RdYlGn`, `BrBG` | ❌ No (red-green) |

You may use **colorblindr**  library to manually select colors.
